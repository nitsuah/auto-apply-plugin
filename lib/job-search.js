import { toIso, ANNUAL_HOURS } from './utils.js';
import { stripHtml } from './jd-parser.js';
// lib/job-search.js
// Multi-source job search aggregation.
//
// Pulls from keyless public job APIs and optional keyed sources, normalizes
// every result to one schema, merges, and de-duplicates. Network fetch is
// injectable so the orchestration is unit-testable without hitting the network.
// Runs in the service worker (which has host permissions and is not subject to CORS).
...
export function normalizeRemoteOkJob(raw = {}) {
  ...
  return {
    ...
    source: 'Remote OK',
    employment_type: '',
    posted: toIso(raw.epoch ? raw.epoch * 1000 : raw.date),
...
export function normalizeReedJob(raw = {}) {
  ...
  return {
    ...
    source: 'Reed',
    employment_type,
    posted: toIso(raw.date || raw.expirationDate),
    tags: [],
...
export function normalizeIndeedJob(item) {
  ...
  // Indeed RSS items often have company in the title or description
  // Try to extract company from description or use a fallback
  const descText = String(description || '');
  const companyMatch = descText.match(/at\s+([A-Z][A-Za-z0-9\s&.,'-]+)(?:\s+-\s+|\s+\||\s+\(|$)/i);
  const company = companyMatch?.[1]?.trim() || 'Indeed';

  // Extract location from description if possible
  const locationMatch = descText.match(/(?:Location|location)[:\s]+([^,\n]+(?:,\s*[A-Z]{2})?)/i);
  const location = locationMatch?.[1]?.trim() || 'Various';

  // Check for remote
  const remote = /remote|work from home|wfh/i.test(description);

  // Extract salary if present - use parseJobPay for structured output
  const salaryText = descText.match(/\$[\d,]+\s*(?:-|to)\s*\$?[\d,]+\s*(?:per year|\/year|annually|hourly|\/hr)/i)?.[0] || '';
  const parsedSalary = parseJobPay(salaryText || descText);

  // Generate a unique ID from the link
  const jobId = link.split('/').pop()?.split('?')[0] || '';
  const id = `indeed:${jobId || Math.random().toString(36).substr(2, 9)}`;

  const pubDate = item.querySelector('pubDate')?.textContent;

  return {
    id: `indeed:${jobId || Math.random().toString(36).substr(2, 9)}`,
    title: cleanText(title, 160),
    company: cleanText(company, 120),
    ...
    salary_interval: parsedSalary?.hourly ? 'hourly' : 'yearly',
    remote,
    url: link,
    atsLabel: detectAtsLabelFromUrl(link),
    source: 'Indeed',
    employment_type: '',
    posted: pubDate ? toIso(pubDate) : '',
    tags: [],
    description: cleanText(stripHtml(description), 6000),
  };
}
...
export function normalizeLinkedInJob(raw = {}, companyMap = {}) {
  ...
  return {
    ...
    source: 'LinkedIn',
    employment_type: '',
    posted: raw.listedAt ? toIso(raw.listedAt / 1000) : '',
    tags: [],
  ...
  return included
    ...
    .map((item) => {
      ...
      if (normalized.salary) {
        const [min, max] = normalized.salary.split('-').map(Number);
