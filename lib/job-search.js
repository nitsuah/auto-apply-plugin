...
export function normalizeIndeedJob(item) {
  ...
  // Match "Company: Name<br/>" or "Company: Name |" etc., handling <b> tags and <br/>
  const companyMatch = decodedDescription.match(/(?:<b>)?(?:Company|Employer)(?:<\/b>)?[:\s]*([^<|\n]+?)(?:\s*(?:<br\s*\/?>| \| | - |\.$))/i);
  if (companyMatch && companyMatch[1].trim() !== '') {
    company = cleanText(companyMatch[1], 120);
  } else {
    // Fallback: try "Role at Company" format (handles <br>, <br/>, etc.)
    const roleAtMatch = decodedDescription.match(/Role\s+at\s+([^<|\n]+?)(?:\s*(?:<br\s*\/?>| \| | - |\.$))/i);
  ...
  const locationMatch = decodedDescription.match(/Location:<\/b>\s*([^<]+)<br/i);
