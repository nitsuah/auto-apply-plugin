...
export function normalizeIndeedJob(item) {
  ...
  // Detection for "Company: ..." labeled descriptions
  let company = '';
  const labeledCompanyMatch = descText.match(/(?:Company|Employer)[:\s]+([^|\n<]+)/i);
  if (labeledCompanyMatch) {
    company = labeledCompanyMatch[1].trim();
  } else {
    // Fallback to "at <Company>" regex
    const companyMatch = descText.match(/at\s+([A-Z][A-Za-z0-9\s&.,'-]+)(?:\s+-\s+|\s+\||\s+\(|$)/i);
    company = companyMatch?.[1]?.trim() || 'Indeed';
  }
  ...
  // Generate a unique ID from the link (prefer 'jk' parameter, fallback to path)
  ...
  return {
    ...
    tags: [],
    description: cleanText(descText, 6000),
  };
}
  ...
  return included
    ...
    .map((item) => {
      ...
      if (normalized.salary) {
        // Extract salary numbers first
        const salaryParts = normalized.salary.match(/\d+(?:,\d+)?/g) || [];
        if (salaryParts.length >= 1) {
          const min = parseFloat(salaryParts[0].replace(/,/g, ''));
          const max = salaryParts.length > 1 ? parseFloat(salaryParts[1].replace(/,/g, '')) : min;
