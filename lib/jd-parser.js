...
//
// Many career pages (incl. custom company domains) embed a structured
// `JobPosting` in <script type="application/ld+json">. This pure parser turns
// those already-parsed objects into our normalized fields — far more reliable
// than scraping the DOM for salary / employment type / location.

/**
 * @param {object[]} ldObjects  Parsed JSON-LD objects from the page.
 * @returns {null|{title:string, company:string, location:string,
 *   employment_type:string, remote:boolean, salary_range:string,
 *   description:string, url:string, datePosted:string}}
 */
export function parseJobPostingJsonLd(ldObjects = []) {
